...
import { User, Bell, Palette, Shield, Download, Trash2, Monitor, Smartphone, Lock, HelpCircle, CreditCard } from "lucide-react";
    ...
    <AppShell>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr] gap-6">
          <nav className="space-y-1">
            {navItems.map(item => {
              ...
              return (
                <button key={item.id} onClick={() => setSection(item.id)} className={cn("flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium transition-colors", section === item.id ? "bg-surface-subtle text-brand-700" : "text-ink-secondary hover:bg-surface-muted")}>
                  <Icon className="h-4 w-4" /> {item.label}
                </button>
              );
            })}
            <a href="/subscription" className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm font-medium text-ink-secondary hover:bg-surface-muted">
              ...
              <Card>
                ...
                <CardContent className="space-y-5">
                    ...
                    <div className="grid grid-cols-7 gap-2">
                      {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map(day => {
                        ...
                        return (
                          ...
                          }} className={cn("py-2 rounded-md border text-xs font-medium transition", active ? "border-brand-500 bg-brand-50 text-brand-700" : "border-surface-border")}>{day}</button>
                        );
                      })}
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div><Label>Weekly Goal (hours)</Label><Input type="number" value={profile.weeklyGoal} onChange={e => updateProfile({ weeklyGoal: Number(e.target.value) })} /></div>
                    <div><Label>Default Study Mode</Label>
                      <select value={profile.defaultStudyMode} onChange={e => updateProfile({ defaultStudyMode: e.target.value })} className="flex h-9 w-full rounded-md border border-surface-border bg-white px-3 text-sm">
                        <option>Focus</option><option>Balanced</option><option>Intensive</option>
                      </select>
                    </div>
                    <div><Label>Question Practice Mode</Label>
                      <select value={profile.questionPracticeMode} onChange={e => updateProfile({ questionPracticeMode: e.target.value })} className="flex h-9 w-full rounded-md border border-surface-border bg-white px-3 text-sm">
                    ...
                    {deleteStep === 0 ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button variant="red">Delete Forever</Button>
                        </div>
                      </div>
                    )}
                  </CardContent>
              ...
              <>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex items-center justify-between p-3 rounded-md border border-surface-border">
                      <div className="flex items-center gap-3"><Monitor className="h-5 w-5 text-ink-tertiary" /><div><p className="text-sm font-medium text-navy-700">Windows • Chrome</p><p className="text-xs text-ink-tertiary">Active now</p></div></div>
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {(['light', 'dark', 'system'] as const).map(t => (
                      <button key={t} onClick={() => setTheme(t)} className={cn("p-4 rounded-md border text-center capitalize", theme === t ? "border-brand-500 bg-brand-50" : "border-surface-border")}>{t}</button>
                    ))}
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <div className="space-y-2">
                      {['How does StepSync calculate readiness?', 'How does adaptive planning work?'].map(q => (
      ...
      <Modal isOpen={exportModal} onClose={() => setExportModal(false)} title="Export Your Data">
        <div className="p-6 space-y-3">
          <p className="text-sm text-ink-secondary">Select categories to include:</p>
          {['Profile', 'Study Plan'].map(c => <div key={c} className="flex items-center gap-2"><input type="checkbox" defaultChecked /> <span className="text-sm">{c}</span></div>)}
        </div>
        <div className="border-t border-surface-border p-4 flex justify-end gap-2">
          ...
          <Button variant="primary" onClick={() => setExportModal(false)}>Download JSON</Button>
        </div>
      </Modal>
